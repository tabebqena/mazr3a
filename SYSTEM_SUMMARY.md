...
| Env | `PORTAL_CONF`, `PORTAL_LOGS_API=http://logs:8090`; `PORTAL_USERS_DB` (default `/media/portal/users.db`), `PORTAL_NOTIF_DB` (default `/media/portal/notifications.db`) |
| PWA (Android) | **Installable web app** — Chrome → *Install app* → home-screen WebAPK. `/manifest.webmanifest` is built per-request by [`portal/app.py`](portal/app.py) so its icon URLs carry the CURRENT fingerprints (**no CACHING service worker, by design** — a CCTV portal is useless offline and a caching SW would fight the cache-busting policy; the ONLY service worker is the notification-only `/sw.js`, which has NO fetch/cache handler). Icon set [`portal/static/icons/`](portal/static/icons/) is generated from the single brand source [`portal/static/favicon.svg`](portal/static/favicon.svg) by [`dev_scripts/make_portal_pwa_icons.sh`](dev_scripts/make_portal_pwa_icons.sh). Install UI = `#install-banner` + `#nav-install` (`app.js`, `beforeinstallprompt`). No new port/compose change. See [`plans/portal-android-pwa.md`](plans/portal-android-pwa.md). |
| Notifications | Feed store [`portal/notifstore.py`](portal/notifstore.py) (`media/portal/notifications.db`); a fire-alert watcher turns NEW Firewatch alerts into items (baselines on first run; **one item per camera per 3-minute burst**, dedup key `fire:<camera>:<bucket>`), and an admin can publish a system message. API `GET /api/notifications`, `POST /api/notifications/read`, `POST /api/admin/notifications`. The SPA **requires** browser Notification permission to run (`#notify-gate`); `tab_notifications` is a grantable tab. Browser alerts go through the notification-only SW `/sw.js` (`GET /sw.js`, stable + `no-cache`); no Web Push. Config: `NOTIFY_POLL_S`, `NOTIFY_RETENTION_DAYS`. See [`plans/portal-notifications.md`](plans/portal-notifications.md). |
...
| `portal` | [`portal/`](portal/) (+ [`portal/notifstore.py`](portal/notifstore.py) notification feed, [`portal/static/sw.js`](portal/static/sw.js) notification-only worker) + [`config/portal.conf`](config/portal.conf); Android-PWA icons via [`dev_scripts/make_portal_pwa_icons.sh`](dev_scripts/make_portal_pwa_icons.sh) → [`portal/static/icons/`](portal/static/icons/) |
...
| [`config/stores/portal.conf`](config/stores/portal.conf) | Portal data-store profile (`media/portal/usage.db` + `media/portal/users.db` + `media/portal/notifications.db`; all hard-protected, so retention is **in-app** — `USAGE_RETENTION_DAYS`, `NOTIFY_RETENTION_DAYS`) |
| [`mosquitto/config/mosquitto.conf`](mosquitto/config/mosquitto.conf) | MQTT broker |
| [`config/portal.conf.example`](config/portal.conf.example) | Portal template (secret + tunables; the `user` lines are only a first-run seed) + `PORTAL_USERS_DB` / `PORTAL_USAGE_DB` / `PORTAL_NOTIF_DB` |
...
- portal data adds **no image store**: the portal owns three small DBs under `media/portal/` — `usage.db` (per-user bandwidth; **row** expiry owned by the portal via `USAGE_RETENTION_DAYS`, default 180, pruned in-app by [`portal/usage.py`](portal/usage.py)), `users.db` (runtime user accounts, see [`portal/userstore.py`](portal/userstore.py)) and `notifications.db` (the notification feed; row expiry via `NOTIFY_RETENTION_DAYS`, default 90, pruned in-app by [`portal/notifstore.py`](portal/notifstore.py)). All are hard-protected from deletion, so the `portal.conf` profile can free nothing.
