...
C_CONFIG = code([
    ...
    "\n",
    "# --- training sources: LARGE base + small sets MERGED IN (so they are not missed) ---\n",
    "SOURCES = [\n",
    "    # 1) the LARGE base (downloaded by Colab from HuggingFace)\n",
    "    {\n",
    "        'id': 'fireviewer',\n",
...
C_WAIT = code([
    ...
    "t0 = time.time()\n",
    "n = last = 0\n",
    "state = 'alive' if os.path.exists(PIDFILE) else 'no pid'\n",
    "while True:\n",
    "    rows = []\n",
    ...
    "        last = n\n",
    "    if n >= EPOCHS or state in ('gone', 'ZOMBIE (exited)', 'no pid'):\n",
    "        break\n",
    ...
    "if n >= EPOCHS:\n",
    "    print('-> training completed: collect the candidate with Cell 13')\n",
    "elif state == 'alive':\n",
    "    print('-> still running (WAIT_MAX_MIN reached): re-run this cell to keep waiting')\n",
    "elif state == 'no pid':\n",
    "    print('-> no live trainer process (the VM was likely reset). Check Cell 10, then resume with Cell 12.')\n",
    "else:\n",
    "    print('-> trainer exited early (%s). Check Cell 10 (status) / Cell 12 (resume).' % state)\n",
])
