...
C_BUNDLE = code([
    ...
    "\n",
    "# allow the bundle at the shared root OR inside THIS run's folder (e.g. .../v3/colab_upload.zip)\n",
    "if not os.path.exists(DRIVE_ZIP) and os.path.exists(DRIVE_DIR + '/' + VERSION + '/colab_upload.zip'):\n",
    "    DRIVE_ZIP = DRIVE_DIR + '/' + VERSION + '/colab_upload.zip'\n",
    "\n",
...
C_SOURCES = code([
    ...
    "    r = subprocess.run([sys.executable, prep, '--config', cfg_path, '--out', RAW,\n",
    "                        '--cache', CACHE, '--scripts', UPLOAD + '/scripts'],\n",
    "                       capture_output=True, text=True)\n",
    "    print(r.stdout)\n",
    "    if r.returncode != 0:\n",
    "        print(r.stderr, file=sys.stderr)\n",
    "        raise SystemExit('prep_fire_scratch_dataset.py exited ' + str(r.returncode))\n",
...
C_DEDUP = code([
    "# Cell 6 - dedup (self + vs EVERY previous run's fingerprints), then re-render the train\n",
    "# duplicates that dedup removed (dev_scripts/augment_fire_train.py).\n",
    ...
    "\n",
    "    # merge EVERY previous run's fingerprint dirs into one local dir per side (v1 + v2 + ...)\n",
    "    prev_train = '/content/prev_train_fp'\n",
    "    prev_test  = '/content/prev_test_fp'\n",
    "    for v in PREV_VERS:\n",
    "        for src_suffix, dst in (('/fingerprints/train', prev_train),\n",
    "                                ('/fingerprints/test', prev_test)):\n",
    "            src = DRIVE_DIR + '/' + v + src_suffix\n",
    "            if os.path.isdir(src):\n",
    "                os.makedirs(dst, exist_ok=True)\n",
    "                for f in os.listdir(src):\n",
    "                    if f.endswith('.jsonl'):\n",
    "                        _sh.copy2(os.path.join(src, f), os.path.join(dst, v + '__' + f))\n",
    "\n",
    "    d = UPLOAD + '/scripts/dedup_fire_scratch.py'\n",
    "    r = subprocess.run([sys.executable, d,\n",
    "                        '--pool', RAW, '--out', CLEAN,\n",
    ...
    "    if r.returncode != 0:\n",
    "        print(r.stderr, file=sys.stderr)\n",
    "        raise SystemExit('dedup_fire_scratch.py exited ' + str(r.returncode))\n",
    "    print(open(CLEAN + '_report/summary.txt', encoding='utf-8').read())\n",
    "\n",
    "    # re-render the TRAIN duplicates dedup removed (rotation/brightness/contrast/noise/hue/flip).\n",
    "    # Runs BEFORE freeing RAW (it re-reads the removed duplicates from the raw pool).\n",
    "    a = UPLOAD + '/scripts/augment_fire_train.py'\n",
    "    r2 = subprocess.run([sys.executable, a,\n",
    "                         '--pool', RAW, '--clean', CLEAN,\n",
    "                         '--report', CLEAN + '_report'],\n",
    "                        capture_output=True, text=True)\n",
    "    print(r2.stdout)\n",
    "    if r2.returncode != 0:\n",
    "        print(r2.stderr, file=sys.stderr)\n",
    "        raise SystemExit('augment_fire_train.py exited ' + str(r2.returncode))\n",
    "    print(open(CLEAN + '_report/augmentation_summary.txt', encoding='utf-8').read())\n",
    "\n",
    "    _sh.rmtree(RAW, ignore_errors=True)     # clean pool + augmented dups are built; drop the raw pool\n",
