...
C_CONFIG = code([
    ...
    "DRIVE_ZIP  = DRIVE_DIR + '/colab_upload.zip'                # bundle from pack_fire_scratch_colab.sh\n",
    "RUNS       = DRIVE_DIR + '/runs'                            # run artifacts (mirrored every epoch)\n",
    "FPS_TRAIN  = DRIVE_DIR + '/fingerprints/train'              # persistent train fingerprint index\n",
    "FPS_TEST   = DRIVE_DIR + '/fingerprints/test'               # persistent test/val fingerprint index\n",
    "REGISTRY   = DRIVE_DIR + '/registry.json'                   # run history + promote/regress decisions\n",
    "LOGS       = DRIVE_DIR + '/logs'\n",
    "\n",
    ...
    "NAME       = 'scratch-v1'           # run name -> RUNS/<name>/weights/...\n",
    ...
    "    'fire': 'fire', 'Fire': 'fire',\n",
    "    'flame_visible': 'fire',\n",
    "    'smoke': None, 'smoke_visible': None,   # fire-only: smoke boxes dropped -> smoke imgs become background\n",
    "    'other': None, 'default': None,\n",
    "}\n",
    "\n",
    "# --- base model (COCO-pretrained = the clean model) ---\n",
    "BASE_MODEL = 'yolo11s.pt'              # yolo11n faster / yolo11s better; yolo26s = prod-arch parity\n",
    "\n",
    "# --- external training sets (downloaded from their own sources, never re-uploaded) ---\n",
    "SOURCES = [\n",
    "    {\n",
    "        'id': 'fireviewer',\n",
    "        'type': 'huggingface_fireviewer',\n",
    ...
    "    },\n",
    "    # our CCTV domain test - HELD OUT (never trained):\n",
    "    # {'id': 'cctv_test', 'type': 'yolo_dir', 'path': UPLOAD + '/domain_test', 'role': 'test'},\n",
    "    # our domain hard negatives -> train as background:\n",
    "    # {'id': 'negatives', 'type': 'yolo_dir', 'path': UPLOAD + '/negatives', 'role': 'negatives'},\n",
    "    # a Roboflow-style zip source (point to the same sets you downloaded here):\n",
    "    # {'id': 'abonia', 'type': 'zip_url', 'url': 'https://.../fire-8.zip', 'download': 'wget',\n",
    "    #  'layout': 'roboflow', 'class_map': {'Fire': 'fire', 'default': None, 'smoke': None}},\n",
    "]\n",
    "\n",
    "# --- dedup ---\n",
    ...
    "\n",
    "# --- training (scratch: freeze 0, standard YOLO lr) ---\n",
    "EPOCHS      = 100\n",
    "BATCH       = 16    # 32 on an L4, 64+ on an A100; 8 if a T4 OOMs\n",
    "IMGSZ       = 640   # matches production input\n",
    "FREEZE      = 0     # 0 = full train (scratch). Raise only for a later fine-tune.\n",
...
C_DEDUP = code([
    ...
    "                    '--prev-train-index', FPS_TRAIN,\n",
    "                    '--prev-test-index', FPS_TEST,\n",
    "                    '--run-name', NAME,\n",
...
C_WAIT = code([
    ...
    "while True:\n",
    "    rows = []\n",
    "    for _p in (os.path.join(run_local, 'results.csv'), os.path.join(run_drive, 'results.csv')):\n",
    ...
    "print('-> finished/stopped: collect with Cell 13' if\n",
    "      (n >= EPOCHS or state in ('gone', 'ZOMBIE (exited)')) else\n",
    "      '-> still running: re-run this cell to keep waiting')\n",
])
...
C_COLLECT = code([
    ...
    "out_pt = os.path.join(DRIVE_DIR, NAME + '.pt')\n",
    ...
    "\n",
    "# 2) evaluate on the held-out test split (val points at test/images)\n",
    ...
    "\n",
    "# 3) registry: append this run, then compare vs the previous best on mAP@50\n",
    "entry = {\n",
    "    'name': NAME,\n",
    ...
    "\n",
    "# 4) promote THIS run's lightweight fingerprints to the persistent Drive index\n",
...
C_NEXT = md([
    "## Next steps\n",
    "\n",
    "1. Pull `%s/<NAME>.pt` from Drive back into the repo and judge it **locally** against the full\n",
    "   matrix (`dev_scripts/compare_fire_models.py`) and the on-domain CCTV audit\n",
    "   (`dev_scripts/test_fire_model.py` on the held-out evidence / `domain_test`).\n",
    ...
    "3. For the **next run**, edit `NAME` + hyperparameters; the new set is automatically deduped\n",
    "   against this run (and every run before it) via `<DRIVE>/fingerprints/*`.\n",
    "4. Per-stage logs live on Drive at `%s/logs/`.\n",
])
