...
#
# Usage:
#   ./dev_scripts/deploy/prep_coco_model.sh [imgsz] [models]
#   models = comma-separated Ultralytics model names (default: all three)
# Examples:
#   ./dev_scripts/deploy/prep_coco_model.sh 640              # export all
#   ./dev_scripts/deploy/prep_coco_model.sh 640 yolo11s      # export only yolo11s
# ============================================================
set -euo pipefail
...
echo "    NOTE: benchmark_app is on PATH in the current image"
echo "    (/usr/local/bin/benchmark_app) - older images had it at"
echo "    /openvino/benchmark_app, which now fails with 'no such file'."
echo "    -t 10 keeps each run to ~10s:"
echo "      docker exec frigate benchmark_app -m /models/coco/yolo11s.onnx -d GPU -api sync -t 10"
echo "      docker exec frigate benchmark_app -m /models/coco/yolo11n.onnx -d GPU -api sync -t 10"
echo "      docker exec frigate benchmark_app -m /models/coco/yolov8s.onnx -d GPU -api sync -t 10"
