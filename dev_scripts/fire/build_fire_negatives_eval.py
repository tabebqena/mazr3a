...
model-training/default-other/ set) are copied into a self-contained eval split so a
detector can be audited purely for fire/smoke false positives.

Output layout (git-ignored under model-training/*):
    <eval-dir>/
...
Usage:
    python dev_scripts/fire/build_fire_negatives_eval.py --src model-training/default-other
        [--eval-dir model-training/eval/negatives] [--names fire,other,smoke]

Then run a pure FP audit:
    .venv/bin/python dev_scripts/fire/test_fire_model.py models/fire/best.pt \
        model-training/eval/negatives/data.yaml --out model-training/eval/negatives/results
"""
import argparse
import os
import shutil
...
def main():
    ...
    ap.add_argument("--src", required=True,
                    help="dir of curated negative images (e.g. model-training/default-other)")
    ap.add_argument("--eval-dir", default="model-training/eval/negatives")
