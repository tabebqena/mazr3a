...
result can feed dev_scripts/fire/test_fire_model.py / analyze_fire_dataset.py.

USAGE
-----
  # 1) index the real training pool once (cache it so later runs are fast):
  python dev_scripts/fire/dedup_images.py --candidate X \
  ...
  python dev_scripts/fire/dedup_images.py --candidate .../fire-8 --tag abonia --internal

  # 3) dedup against the cached training pool and write the clean set:
  python dev_scripts/fire/dedup_images.py --candidate .../fire-8 --tag abonia --out \
...
dev_scripts/fire/analyze_overlap.py (imported by adding this dir to sys.path).
"""
import argparse
import csv
import hashlib
...
# --------------------------------------------------------------------------- #
# class-name parser (tiny, mirrors dev_scripts/fire/analyze_fire_dataset.py)
# --------------------------------------------------------------------------- #
...
def _write_data_yaml(out_root, layout_splits, names):
    ...
    lines = ["# generated by dev_scripts/fire/dedup_images.py - labels copied verbatim",
