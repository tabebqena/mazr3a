...
YOLO tree (the dedup passes run afterwards in dev_scripts/fire/dedup_fire_scratch.py).
...
------------
  huggingface_fireviewer  FireViewer Fire & Smoke Detection Corpus v1 (parquet). Downloaded with
                          ...
                          and converted by ``dev_scripts/fire/prep_fireviewer_dataset.py`` (bundled).
...
def write_data_yaml(out, classes):
    ...
    with open(path, "w", encoding="utf-8") as fh:
        fh.write("# merged raw pool - dedup with dev_scripts/fire/dedup_fire_scratch.py\n")
        fh.write("path: %s\n" % out.replace("\\", "/"))
