...
fire model (models/fire/) and the eval harness (dev_scripts/fire/test_fire_model.py)
...
-----
  ...
  # data.yaml is directly usable by dev_scripts/fire/test_fire_model.py (D-Fire has
  # no 'valid' split; the held-out test split plays the val role, as in Phase 2):
  .venv/bin/python dev_scripts/fire/remap_dataset_classes.py \
...
def write_data_yaml(out_root, splits_present, names, test_as_val=False):
    lines = [
        "# generated by dev_scripts/fire/remap_dataset_classes.py (class remap applied)",
...
def main():
    ...
    ap.add_argument("--test-as-val", action="store_true",
                    help="D-Fire has no 'valid' split; write BOTH val: and test: -> test/images "
                         "so the output data.yaml is directly usable by dev_scripts/fire/test_fire_model.py "
                         "(Phase 3 clean-eval).")
    args = ap.parse_args()
