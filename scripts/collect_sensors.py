#!/usr/bin/env python3
"""Live lm-sensors reading + monitoring helpers for the host scripts.

Single consolidated module for the host monitoring stack (absorbed the old
monitor_lib.py; see plans/merge-monitor-lib-into-collect-sensors.md).

Provides:
  - load_conf():         read the KEY=VALUE config (config/telegram.conf)
                         holding Telegram credentials + tunables (git-ignored;
